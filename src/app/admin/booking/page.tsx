...
export default function AdminBookingPage() {
  ...
  const addToCart = (item: Item) => {
    setCart((prev) => {
      ...
      return [
        ...prev,
        { id: item.id, name: item.name, price: item.price, qty: 1, isPackage: false },
      ];
    });
  };
  ...
  const decreaseQty = (id: number) => {
    setCart((prev) =>
      prev
        .map((item) => (item.id === id && !item.isPackage ? { ...item, qty: item.qty - 1 } : item))
  ...
  const removeItem = (id: number, isPackage?: boolean) => {
    setCart((prev) => prev.filter((item) => !(item.id === id && item.isPackage === isPackage)));
  };
  ...
  try {
    ...
    const res = await api.post("/admin/createbooking", {
      ...
      items: cart.flatMap((item) => {
        if (item.isPackage) {
          return item.packageItems?.map((pkgItem) => ({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 bg-slate-50 min-h-screen text-slate-800 antialiased">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold tracking-tight text-slate-900">Pilih Item</h2>
              ...
              >
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-700 mt-1">
                    Rp {formatRupiah(item.price)} <span className="text-xs font-normal text-slate-400">/ hari</span>
                  </p>
                </div>
                <div className="mt-4 flex items-center justify-between w-full border-t border-slate-50 pt-2">
                  <span className={`text-xs px-2 py-0.5 rounded-md font-medium ${item.stock > 0 ? 'bg-slate-100 text-slate-600' : 'bg-rose-50 text-rose-600'}`}>
                    Stok: {item.stock}
                  </span>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold tracking-tight text-slate-900">Paket Hemat</h2>
            <span className="text-xs font-medium text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full">Bundling</span>
          </div>
                  ...
                  <div className="text-xs mt-3 space-y-1.5 text-indigo-950/70">
                    {pkg.package_items?.map((i) => (
                      <div key={i.id} className="flex items-center gap-1.5">
                        ...
                        <span>{i.item.name} <strong className="text-indigo-900">x{i.quantity}</strong></span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-5 h-fit sticky top-6">
        <h2 className="font-bold text-xl tracking-tight text-slate-900 pb-2 border-b">Detail Booking</h2>

        {/* SECTION KERANJANG (CART) */}
        <div>
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Item Terpilih</label>
          <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1 subtle-scrollbar">
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <div key={`${item.id}-${item.isPackage}`} className={`p-3 rounded-xl border flex flex-col gap-2 ${item.isPackage ? 'bg-indigo-50/40 border-indigo-100' : 'bg-slate-50 border-slate-100'}`}>
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-1.5">
                        <p className="text-sm font-semibold text-slate-900">{item.name}</p>
                        {item.isPackage && (
                          <span className="bg-indigo-100 text-indigo-700 font-bold text-[8px] px-1.5 py-0.5 rounded uppercase tracking-wider">Paket</span>
                        )}
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">
                        Rp {formatRupiah(item.isPackage ? (item.price - (item.discountPrice || 0)) : item.price)} × {duration} hari
                      </p>
                    </div>
                    <button 
                  ...
                  <div className="flex justify-between items-center pt-1.5 border-t border-slate-200/60 mt-1">
                    <span className="text-xs font-bold text-emerald-600">
                      Rp {formatRupiah((item.isPackage ? (item.price - (item.discountPrice || 0)) : item.price) * item.qty * duration)}
                    </span>
                    
                    {item.isPackage ? (
                      <span className="text-[10px] font-bold text-indigo-500 italic px-2 bg-indigo-50 py-0.5 rounded-md border border-indigo-100/40">Qty Terkunci (1)</span>
                    ) : (
                      <div className="flex items-center gap-1 bg-white border rounded-lg p-0.5 shadow-sm">
                        ...
                        <span className="text-xs font-semibold px-2 min-w-[20px] text-center">{item.qty}</span>
                        <button 
        ...
        <div className="space-y-3 pt-2">
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Informasi Pelanggan</label>
          <div>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] font-bold text-slate-500 block mb-1 ml-1">Mulai</label>
              <input
                type="date"
                value={startDate}
            ...
            <div>
              <label className="text-[10px] font-bold text-slate-500 block mb-1 ml-1">Selesai</label>
              <input
                type="date"
                value={endDate}
          ...
          <div className="flex justify-between items-center bg-slate-50 border p-2.5 rounded-xl text-xs font-medium text-slate-600">
            ...
            <span className="font-bold text-slate-900 bg-white px-2 py-0.5 rounded border shadow-sm">{duration} Hari</span>
          </div>
        </div>

        {/* SECTION METODE PEMBAYARAN */}
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Metode Pembayaran</label>
          <select
            value={paymentMethod}
