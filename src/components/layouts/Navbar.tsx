...
export function Navbar({ mounted, user, token, logout }: any) {
  ...
  return (
    <nav className="container mx-auto px-6 h-20 flex items-center justify-between border-b border-slate-100">
      ...
      <div className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-700">
        ...
        {/* Menu ini hanya muncul jika user terverifikasi */}
        {mounted && token && user && (
        ...
        {mounted && (
          token ? (
            ...
            <Button 
              variant="outline" 
              onClick={openAuth} 
              className="relative gap-2 rounded-full border-2 border-slate-200 px-6 h-11 hover:bg-slate-50"
            >
