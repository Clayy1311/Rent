...
export function CartDialog() {
  ...
  const { cart, packages, removeFromCart, updateQuantity, clearCart, removePackage } = useCartStore();
  ...
  const totalPrice = packages.length > 0 
    ? (packages[0].finalPrice || packages[0].final_price || 0) * validDays
    : cart.reduce((total, item) => total + (item.price * item.quantity * validDays), 0);
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="outline" className="relative gap-2 rounded-full border-2 border-slate-200 shadow-sm px-6 h-11 hover:bg-slate-50 transition-all">
          <ShoppingCart className="h-5 w-5 text-slate-600" />
          <span className="text-sm font-semibold text-slate-900">Keranjang</span>
          {(cart.length > 0 || packages.length > 0) && (
            <Badge className="absolute -top-2 -right-2 bg-primary text-white border-2 border-white px-1.5 min-w-[20px]">
              ...
              >
                ...
                <div className="flex flex-col">
                  {date?.from ? (
                    ...
                    <span className="text-sm text-slate-500">Pilih tanggal</span>
                  )}
                  <span className="text-[10px] text-primary font-bold">{validDays} Malam Sewa</span>
                </div>
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0 bg-slate-950 border-slate-800 shadow-2xl" align="center">
              <Calendar
                initialFocus
                mode="range"
        ...
        <ScrollArea className="max-h-[35vh] pr-4 px-1">
          {cart.length === 0 && packages.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {packages.map((pkg) => (
                <div key={pkg.id} className="flex gap-4 items-center bg-primary/10 p-3 rounded-2xl border border-primary/20">
                  <div className="h-10 w-10 bg-primary rounded-xl flex items-center justify-center">
                     <Box className="text-white h-6 w-6" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-xs text-white truncate">{pkg.package_name}</h4>
                    <p className="text-[10px] text-primary font-bold uppercase tracking-tighter">Paket Bundling</p>
                    <p className="text-sm text-white font-black">
                      Rp {(pkg.finalPrice || pkg.final_price || 0).toLocaleString('id-ID')}
                    </p>
                  </div>
                  <button onClick={() => removePackage()} disabled={loading} className="text-slate-600 hover:text-red-400">
              ...
              {cart.map((item) => (
                <div key={item.id} className="flex gap-4 items-center bg-white/5 p-3 rounded-2xl border border-white/5">
                  ...
                  <div className="flex items-center gap-3 bg-slate-950 px-2 py-1 rounded-full border border-slate-800">
                      <button onClick={() => updateQuantity(item.id, item.quantity - 1)} disabled={item.quantity <= 1 || loading} className="text-slate-400 hover:text-white disabled:opacity-30"><Minus size={12}/></button>
                      <span className="text-xs font-bold text-white">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.id, item.quantity + 1)} disabled={loading} className="text-slate-400 hover:text-white"><Plus size={12}/></button>
                  </div>

                  <button onClick={() => removeFromCart(item.id)} disabled={loading} className="text-slate-600 hover:text-red-400">
                    <Trash2 className="h-4 w-4" />
                  </button>
                </div>
              ))}
        ...
        {(cart.length > 0 || packages.length > 0) && (
          <div className="pt-4 border-t border-slate-800">
            <div className="flex justify-between items-center mb-4 px-2">
              <div className="flex flex-col">
                <span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest leading-none">Total Pembayaran</span>
